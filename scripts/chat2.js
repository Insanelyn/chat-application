...
$(function(){
    ...
    let objFirebase = new Firebase("https://chat-gruppen.firebaseio.com/messages2");
    let isOnline = sessionStorage.getItem("isOnline");
    if(isOnline === "isOnline") {
        //list user in users online field
    }
    console.log("is online", isOnline)
    $('#btnSend2').click(clickShipping);
    // $('#imgAvatar').attr('src', sessionStorage.getItem('profileImageURL'));
    $('#userName2').val(sessionStorage.getItem('username'));
    ...
    function clickShipping(){
        ...
        let message = $('#sendBoxMessage2').val();
    ...
    objFirebase.on("child_added", function(snapshot){
        ...
        let style = "";
        if (values.author === $('#userName2').val()) { style = 'me'}
        $('.boxMessageTimeline2').prepend(getTemplate(values.author, values.message, style, values.image));
    });
